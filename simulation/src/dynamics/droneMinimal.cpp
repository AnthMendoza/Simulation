...
void drone::initDrone(string& droneConfig){
...
void drone::transposeEntities(const Quaternion& quant){
...
void drone::rotationHelper(const Quaternion& q){
...
void drone::rotateLocalEntities(const Quaternion& quant){
...
void drone::updateState(float time,std::optional<controlPacks::variantPackets> controlInput){
...
void drone::setEntitiesPose(const poseState& pose) {
...
void drone::publish_simulation() {
    ...
    auto& gps = field.gps[0].emplace(); // emplace defaults constructs the underlying struct wrapped in std::optional
