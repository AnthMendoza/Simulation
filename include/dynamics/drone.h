...
class droneBody :  public Vehicle{
    ...
    */
    ...
    droneBody(  std::unique_ptr<battery> battery,
                std::unique_ptr<droneControl> control);
    ~droneBody();
    ...
    inline vector<float> updateController(){
        return controller->update(getPositionVector(),getPose(),getVelocityVector(),mass,gravitationalAcceleration,getTime(),totalThrustLimit);
    }
    ...
    */
    ...
    inline void droneDisplay() const {
        ...
        std::cout << "requested Moments (mx,my,mz): ("
                  << controller->moments[0] << ", "
                  << controller->moments[1] << ", "
                  << controller->moments[2] << ")\n";
