...
namespace constants{
    ...
    std::array<float,3> initPosition = {0,0,100};
    ...
    std::array<float,3> initVehicleState = {0,1,1}; // this the vehicle vector, its direction is the directionthe nose points. it will be normilized, magnatiude is meain
    std::array<float , 3> MOI = {2,2,2};

    float centerOfPressure = 1;
    float timeStep = .01; //seconds
