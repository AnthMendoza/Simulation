...
droneBody::droneBody(std::unique_ptr<battery> battery,
                     std::unique_ptr<droneControl> control):
                     droneBattery(std::move(battery)),
...
void droneBody::allocatorHelper(){
    ...
    vector<float> coef = {  propellers[0]->powerCoefficient,
                            -propellers[1]->powerCoefficient,
                            propellers[2]->powerCoefficient,
                            -propellers[3]->powerCoefficient};
    for(int i = 0 ; i < propellers.size();i++){
